...
profile is the only source for the name and the country:

- `users.display_name` is **never** read for this. It holds the synthetic
  address signup mints (`22790123456@phone.afrinext.local`), which must never
  reach a payment provider.
- Better Auth's `user.name` is **never** read for this. It holds the phone
  string.
- The country is **never inferred** — not from the phone's calling code, not
